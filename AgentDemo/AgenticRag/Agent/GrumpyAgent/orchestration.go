...
func BuildGrumpyAgent(ctx context.Context) (r compose.Runnable[map[string]any, *schema.Message], err error) {
	const (
		DemoAgent        = "DemoAgent"
	...
	r, err = g.Compile(ctx, compose.WithGraphName("GrumpyAgent"))
	if err != nil {
		return nil, err
	}
	return r, err
